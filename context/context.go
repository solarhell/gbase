...
// variable alias
var (
	Canceled         = stdcontext.Canceled
	DeadlineExceeded = stdcontext.DeadlineExceeded
)
...
func New(gctx stdcontext.Context, env Env, logger Logger) Context {
	if gctx == nil {
		gctx = stdcontext.Background()
	}
	if env == nil {
		env = NewEnv()
	}
...
func (ctx *context) Reborn() Context {
	return ctx.RebornWith(stdcontext.Background())
}

func (ctx *context) RebornWith(gctx stdcontext.Context) Context {
	if gctx == nil {
		gctx = stdcontext.Background()
	}
	var newctx = ctx.fork("", "")
...
func (ctx *context) WithCancel() (Context, CancelFunc) {
	var newctx = ctx.fork("", "")
	var newgctx, f = stdcontext.WithCancel(newctx.gctx)
	newctx.gctx = newgctx
	return newctx, f
}
...
func (ctx *context) WithDeadline(d time.Time) (Context, CancelFunc) {
	var newctx = ctx.fork("", "")
	var newgctx, f = stdcontext.WithDeadline(newctx.gctx, d)
	newctx.gctx = newgctx
	return newctx, f
}
...
func (ctx *context) WithTimeout(timeout time.Duration) (Context, CancelFunc) {
	var newctx = ctx.fork("", "")
	var newgctx, f = stdcontext.WithTimeout(newctx.gctx, timeout)
	newctx.gctx = newgctx
	return newctx, f
}
...
func (ctx *context) WithValue(key, value interface{}) Context {
	var newctx = ctx.fork("", "")
	newctx.gctx = stdcontext.WithValue(newctx.gctx, key, value)
	return newctx
}
